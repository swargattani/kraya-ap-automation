...
export async function autoMatchPO(invoice) {
  const candidates = [];

  // Priority 1: explicit PO reference on invoice
  if (invoice.poRef) {
    const byRef = await PO.findOne({ poNo: invoice.poRef, status: 'open' });
    if (byRef) candidates.push({ po: byRef, score: 100 });
  }

  // Priority 2: same vendor GSTIN + open status
  if (invoice.vendorGSTIN) {
    const byGstin = await PO.find({ vendorGSTIN: invoice.vendorGSTIN, status: 'open' });
    byGstin.forEach(po => {
  ...
  if (!candidates.length && invoice.vendorName) {
    ...
    const byName = await PO.find({ vendorName: nameRe, status: 'open' });
    byName.forEach(po => {
      const amtDiff = invoice.totalAmount && po.totalAmount
        ? Math.abs(invoice.totalAmount - po.totalAmount) / po.totalAmount
        : 1;
